...
//2逼列表，只支持開頭和結尾的*號，不支持中間的*號
var _2Blist=["*google*","*facebook*"];

function FindProxyForURL(url, host) {
    if (hasOwnProperty.call(whitelist, host)) {
        return direct;
    }
    for(var i=0;i<_2Blist.length;++i){
        var keyword=_2Blist[i].replace(/\*/g,"");
        if(host.indexOf(keyword)>-1)return proxy;
    }
    var suffix;
